...
/**
 * UUID bean property validator implementation.
 * 
 * @author nichollsmc 
 * Last updated by: nichollsmc
 */
public class UUIDValidator implements BeanPropertyValidator {

    @Override
    public boolean validate(String uuid) {
        return new CommonBarcodeAndUUIDValidatorImpl().validateUUIDFormat(uuid);
    }

    @Override
    public String getMessagePropertyKey() {
        return "{gov.nih.nci.ncicb.tcga.dcc.common.bean.validation.AssertionType.UUIDValidator.message}";
    }
}