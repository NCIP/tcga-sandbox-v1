...
COLUMN per_barcode HEADING 'PER BARCODE'
COLUMN start_date HEADING 'START'
COLUMN end_date HEADING 'END'
BREAK ON experiment 
select v.center,v.platform,v.experiment,v.source_file_type,v.elapsed,v.barcodes, v.elapsed/v.barcodes as per_barcode, v.start_date, v.end_date
FROM (
select c.domain_name as center, p.platform_name as platform, d.experiment_id as experiment, d.source_file_type, cast(max(f.load_end_date) as timestamp) - cast(min(f.load_start_date) as timestamp) as elapsed,
...
group by d.experiment_id, d.source_file_type,c.domain_name,p.platform_name
order by d.experiment_id, d.source_file_type) v;