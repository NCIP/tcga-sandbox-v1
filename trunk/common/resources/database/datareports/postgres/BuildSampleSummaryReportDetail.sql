...
$BODY$
	...
	*/
	...
	(tumor_abbreviation, center_id, center_name, center_type, platform, biospecimen_id, barcode, sample, portion_analyte, is_valid, is_viewable, date_received)
	SELECT distinct t.tumor_abbreviation, c.id as center_id, c.center_name, m.center_type, p.platform_alias as platform, bb.biospecimen_id, bb.barcode, bb.sample, bb.portion_analyte, 
	       bb.is_valid, bb.is_viewable, min(a.date_added)  as received_date
	...
	GROUP BY t.tumor_abbreviation, c.id , c.center_name, m.center_type, p.platform_alias, bb.biospecimen_id, bb.barcode, bb.sample, bb.portion_analyte, 
	...
	*/
	...
	(tumor_abbreviation,center_id,center_name,center_type,biospecimen_id,barcode,sample,portion_analyte, ship_date) 
	SELECT distinct t.tumor_abbreviation,c.id as center_id,c.center_name,m.center_type,b.biospecimen_id,b.barcode,b.sample,b.portion_analyte, s.shipdate
	FROM   biospecimen_breakdown_all b 
	       INNER JOIN biospecimen_to_archive ba ON b.biospecimen_id = ba.biospecimen_id 
	       INNER JOIN  Archive_info a ON ba.archive_id = a.id 
			AND a.is_latest = 1
			AND a.deploy_status = 'Available'
	       INNER JOIN tumor_info t ON a.tumor_id = t.id 
	       INNER JOIN center_bcr_center_map m ON b.bcr_center_id = m.bcr_center_id
	       INNER JOIN center_info c ON m.center_id = c.id 
	       LEFT OUTER JOIN biospecimen_shipdate s ON b.biospecimen_id=s.biospecimen_id
	WHERE  b.is_valid=1
	AND b.is_viewable=1;
	...
	*/
	...
	(SELECT COUNT(distinct center_sample) FROM 
	(SELECT  s.sample,l.sample as center_sample,l.center_name,l.tumor_abbreviation,l.center_type,l.portion_analyte,l.platform
