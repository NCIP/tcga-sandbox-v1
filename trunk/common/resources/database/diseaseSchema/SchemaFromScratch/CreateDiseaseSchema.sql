...
--------------------------------------------------------
  ...
  DROP VIEW DATA_SET_V;
  DROP VIEW DISEASE_V;
  DROP VIEW NUMBER_ANOMALIES_GENE;
  DROP VIEW PATHWAY_TO_GENE;
  DROP VIEW SBG_MART;
  DROP VIEW SBG_PATIENT_PERCENTAGE;
...
--------------------------------------------------------
   ...
   (	MAF_INFO_ID 			NUMBER(38,0)	NOT NULL,
        MAF_KEY_ID			NUMBER(38,0)    NOT NULL,
        FILE_ID				NUMBER(38,0)    NOT NULL,
	HUGO_SYMBOL 			VARCHAR2(50)	NOT NULL,
...
--------------------------------------------------------
  ...
  CREATE BITMAP INDEX L4_ANOMALYVAL_GE_IDX ON L4_ANOMALY_VALUE (GENETIC_ELEMENT_ID) 
   LOCAL;
--------------------------------------------------------
--  DDL for Index L4_ANOMALYVAL_SAMPLE_IDX
--------------------------------------------------------

  CREATE BITMAP INDEX L4_ANOMALYVAL_SAMPLE_IDX ON L4_ANOMALY_VALUE (SAMPLE_ID) 
   LOCAL;
--------------------------------------------------------
...
ALTER TABLE maf_info ADD (
   ...
   CONSTRAINT fk_maf_info_file
   FOREIGN KEY (file_id) 
   REFERENCES file_info(file_id)
);

CREATE INDEX maf_info_file_idx ON maf_info (file_id);
CREATE INDEX maf_info_mafkey_idx ON maf_info (maf_key_id);
...
CREATE OR REPLACE VIEW shipped_biospecimen_aliquot
AS
...
CREATE OR REPLACE VIEW AVG_COL_LEN
(
   TABLE_NAME,
   COLUMN_NAME,
   AVG_COL_LEN,
