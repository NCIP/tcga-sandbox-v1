...
class com.fusioncharts.core.charts.StackedColumn3DLineDYChart extends DualYAxis3DChart 
{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								if(!anyAnchorVisible){
									if(setAlpha > 0 && setAnchorAlpha > 0 && setAnchorSides > 2 && setAnchorRadius != 0 && !isNaN(setValue)){
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		labelMetrics.canvasPadding = this.params.canvasPadding + (this.config.columnWidth / 2) + (this.num > 2 ? 0 : this.config.columnSpace);
		...
		if(this.params.XTLabelManagement){
			...
			var plotSpaceRatio:Number;
			if(this.num > 2){
				plotSpaceRatio = this.params.plotSpacePercent / 100;
			}else{
				plotSpaceRatio = (this.num * this.params.plotSpacePercent / 100) / (this.num - this.params.plotSpacePercent / 100 + 1);
			}
			...
			var newCanvasWidth:Number
			if(this.num > 2){
				newCanvasWidth = metricsObj.totalInterPlotWidth + newColumnWidth;
			}else{
				newCanvasWidth = ( (this.num + plotSpaceRatio) / (this.num - 1) * metricsObj.totalInterPlotWidth);
			}
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			{
				if(this.params.XTLabelManagement){
					this.categories [i].x = this.elements.canvasBg.x + (amountOfDiff/2) + (newSpaceWidth * (i - 1)) + ((i - 0.5) * newColumnWidth) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
				}else{
	...
	*/
	private function drawHeaders ()
	{
		...
		if (this.params.xAxisName != "")
		{
			...
			var xAxisNameObj : Object = createText (false, this.params.xAxisName, this.cMC, this.dm.getDepth ("XAXISNAME") , this.elements.canvasBase.x + (this.elements.canvasBase.w / 2) , this.elements.canvas.toY + this.params.labelPadding + this.config.labelAreaHeight + this.params.xAxisNamePadding, 0, xAxisNameStyleObj, true, this.elements.xAxisName.w, this.elements.xAxisName.h);
			//Apply animation
			if (this.params.animation)
			{
