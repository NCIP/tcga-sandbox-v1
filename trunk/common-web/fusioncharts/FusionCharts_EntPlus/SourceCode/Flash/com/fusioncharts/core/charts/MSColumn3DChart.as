...
class com.fusioncharts.core.charts.MSColumn3DChart extends SingleYAxis3DChart 
{
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		labelMetrics.canvasPadding = this.params.canvasPadding + ((this.config.columnWidth * this.numDS) / 2) + (this.num > 2 ? 0 : this.config.blockSpace);
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				{
					if(this.params.XTLabelManagement){
						this.dataset [j].data [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newBlockSpaceWidth * (i - 1)) + (newBlockWidth * (i - 1)) + (newColumnWidth * (j - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
					}else{
