				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						if (isNaN(setValue)==false){
							...
							**/
							var setColor : String = getFV ( formatColor (atts ["color"]), "");
	...
	*/
	private function calculatePoints ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			{
				if(this.params.XTLabelManagement){
					this.categories [i].x = this.elements.canvasBg.x + (amountOfDiff/2) + (newSpaceWidth * (i - 1)) + ((i - 0.5) * newBlockWidth) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
				}else{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].parentYAxis == "P")
				{
					...
					{
						if(this.params.XTLabelManagement){
							this.dataset [j].data [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newSpaceWidth * (i - 1)) + (newBlockWidth * (i - 1)) + (newColumnWidth * (colCounter - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
						}else{
