...
class com.fusioncharts.core.charts.MSColumnLine3DChart extends SingleYAxis3DChart 
{
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		labelMetrics.canvasPadding = this.params.canvasPadding + ((this.config.columnWidth * this.numColDS) / 2) + (this.num > 2 ? 0 : this.config.blockSpace);
		...
		if(this.params.XTLabelManagement){
			...
			if(this.num > 2){
				newCanvasWidth = metricsObj.totalInterPlotWidth + newBlockWidth;
			}else{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			{
				if(this.params.XTLabelManagement){
					this.categories [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newBlockSpaceWidth * (i - 1)) + (newBlockWidth * (i - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
				}else{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].renderAs == "COLUMN")
				{
					...
					{
						if(this.params.XTLabelManagement){
							this.dataset [j].data [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newBlockSpaceWidth * (i - 1)) + (newBlockWidth * (i - 1)) + (newColumnWidth * (colCounter - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
						}else{
