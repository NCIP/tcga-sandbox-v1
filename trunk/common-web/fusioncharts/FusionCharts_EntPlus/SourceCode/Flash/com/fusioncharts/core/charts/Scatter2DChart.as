...
class com.fusioncharts.core.charts.Scatter2DChart extends XYPlotChart
{
	...
	*/
	public function render (isRedraw:Boolean) : Void
	{
		...
		} else
		{
			...
			yNumberScales = getNumberScales(this.params.yNumberScaleValue, this.params.yNumberScaleUnit, this.params.yFormatNumberScale);
			// get scales value for x axis 
			xNumberScales = getNumberScales(this.params.xNumberScaleValue, this.params.xNumberScaleUnit, this.params.xFormatNumberScale);
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								if(!anyValidPlot){
									anyValidPlot = (!isNaN(setYV) && (isAnchorVisible || setShowValue == 1))? true : anyValidPlot;
								}
								...
								//If set x value is explicitly define and less than xAxisMinValue then delete xAxisMinValue
								if(!isNaN(this.params.xAxisMinValue) && !isNaN(atts["x"])){
									if(setXV < this.params.xAxisMinValue){
										delete this.params.xAxisMinValue;
									}
								}
								//If set x value is explicitly define and greater than xAxisMaxValue then delete xAxisMaxValue  
								if(!isNaN(this.params.xAxisMaxValue) && !isNaN(atts["x"])){
									if(setXV > this.params.xAxisMaxValue){
										delete this.params.xAxisMaxValue;
									}
								}
							}
						}
	...
	*/
	private function calculateAxisLimits () : Void
	{
		...
		this.getXAxisLimits(this.getMaxXDataValue(), this.getMinXDataValue(), false, !this.params.setAdaptiveXMin);
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.dataset [i].num; j ++)
			{
				...
				this.dataset [i].data [j].displayValue = formatNumber (this.dataset [i].data [j].yv, 
																		...
																		this.params.yAxisValueDecimals, 
																		this.params.forceYAxisValueDecimals, 
				...
				if (this.dataset [i].data [j].toolText == undefined || this.dataset [i].data [j].toolText == "")
				{
					...
					toolText = toolText + (this.params.xAxisLabelMode == "categories" ? this.dataset [i].data [j].xv : (formatNumber (this.dataset [i].data [j].xv ,
														 ...
														 this.params.xAxisValueDecimals, 
														 ...
														 this.params.xNumberSuffix)))+ this.params.toolTipSepChar;
		...
		for (i = 0; i < this.numCat; i ++)
		{
			...
			if (this.categories [i].isValid && this.categories [i].showLabel)
			{
				...
				if (this.params.labelDisplay == "ROTATE")
				{
					...
					labelObj = createText(false, this.categories[i].label, dataLabelMc, i, this.categories[i].x, this.elements.canvas.toY+this.params.labelPadding, this.config.labelAngle, labelStyleObj, true, this.config.wrapLabelWidth, labelWidth);
