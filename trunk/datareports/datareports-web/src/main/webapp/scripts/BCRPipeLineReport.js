...
tcga.riphraph.createFilter = function() {
    ...
    var dateCombo = new Ext.form.ComboBox({
        ...
        listeners: {
			select: function(combo, rec) {
                ...
                var bcrCombo = Ext.getCmp('bcrCombo');
                nodeDataStore.loadUrl('pRepData.json?disease='+diseaseCombo.getValue()+
                        '&bcr='+bcrCombo.getValue()+'&date='+rec.get('id'));
			}
	    }
    });
    ...
    var bcrCombo = new Ext.form.ComboBox({
        name: 'bcrCombo',
        id:  'bcrCombo',
        width: 120,
        store: bcrStore,
        ...
        displayField: 'text',
        valueField: 'id',
        emptyText: 'Select a bcr',
        tpl: '<tpl for="."><div id="bcr{[xindex]}" class="x-combo-list-item">{text}</div></tpl>',
        border: false,
	    autoHeight: true,
        listeners: {
			select: function(combo, rec) {
                ...
                nodeDataStore.loadUrl('pRepData.json?disease='+diseaseCombo.getValue()+
                        '&bcr='+rec.get('id')+'&date='+dateCombo.getValue());
			}
	    }
    });
	...
	new Ext.Panel({
        ...
        {   // Spacer
			...
			listeners: {
				select: function(combo, rec) {
				    ...
				    nodeDataStore.loadUrl('pRepData.json?disease='+rec.get('tumorName')+
                            '&bcr='+bcrCombo.getValue()+'&date='+dateCombo.getValue());
				}
			}
		}, {
			xtype: 'button',
			...
			handler: function() {
                ...
                bcrCombo.setValue(bcrCombo.getStore().getAt(0).get('id'));
