#!/bin/perl
#$Id: jsonval.pl 18048 2013-01-24 16:51:56Z snyderee $
...
ARRAY_ELT:
  ...
  TAG:
	...
	($type eq 'int') && do {
	  ...
	  unless ( is_int($val) ) {
	    fail_row_msg($table,$elt_index,"Tag '$tag' value is not an integer");
	  }
	  next TAG;
	};
	($type eq 'float') && do {
	  ...
	  unless ( is_float($val) ) {
	    fail_row_msg($table,$elt_index,"Tag '$tag' value is not a float");
	  }
	  next TAG;
	};
	($type eq 'date') && do {
