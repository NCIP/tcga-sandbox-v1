...
linkMigrationTest02.pl ($Revision: 17921 $)
...
my @queries = (
	...
	"select distinct f.file_id, f.file_name, f.file_location, f.date_file_stored, f.shared_link, a.archive_name, a.shared_link
		from file_item f, file_item_archive fa, virtual_archive a
		where f.file_location = twofilepath and a.archive_id = fa.archive_id
...
&print_LoH_as_table( $dLoH, $opts{'o'}, $fsep, $keys ) if $opts{'o'};	# print table of retrieved data for debugging purposes
...
foreach my $argHash ( @$dLoH ){											# loop through list of docs/db records defined
	...
	$i++;
	$subquery = $qstrings[0];
	$subquery =~ s/twofilepath/'$argHash->{'twofilepath'}'/;
	my $sth = $dbh->prepare( $subquery );
	print "subquery: $subquery\n" if $opts{'d'};
	my $sharedLink = &execute_query( $subquery, $dbh, $sth );
	my $resultLoH = &parse_query_results( $sharedLink, $qvals[0] );

#In the database, the 'virtual_archive' table is populated with ONE corresponding record.
#The timestamp and information populated should correspond to the submitted archive.

	if ( @$resultLoH > 1 ){
		print "WARNING: Multiple virtual archives associated with file: \"" . $argHash->{'twofilepath'} . "\"\n";
...
#and containing both uppercase/lowercase letters. Every file will have a shared_link.
		...
		&inspect_results( $argHash, $resultLoH->[0] );
	} else {
		print "WARNING: No results for query: $subquery\n";
	}

#The 'file_item_archive' table is populated, and is linked to the 'file_item' and 'virtual archive' tables.

	unless ( $i ){
		&print_hash_by_line( $argHash, "key", \@args2print );
	}
	&print_hash_by_line( $argHash, "value", \@args2print );
}
$dbh->disconnect;
...
####################################################################################################
sub	inspect_results{
	my ( $ref, $query ) = @_;					# ref = reference data from sqlite DB
