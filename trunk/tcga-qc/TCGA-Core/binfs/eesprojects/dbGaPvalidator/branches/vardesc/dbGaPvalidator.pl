#! /bin/perl -w
####################################################################################################
my $revision = '$Revision: 15762 $';
$revision =~ s/^\$\s*(.+)\s*\$$/$1/;
our $rel_date       = '$Date: 2012-02-28 01:10:14 -0500 (Tue, 28 Feb 2012) $';
...
my %usage   = (                                                 # init paras for getopts
    ...
    'D' => {
        'type'     => "string",
        'usage'    => "file containing user-defined variable descriptions (to replace \"null\"s)",
        'required' => 0,
        'init'     => 0,
    },
    'F' => {
...
my $vardesc = &read_2col_data2hash_fromfile( $opts{'D'} ) if $opts{'D'}; # read user-defined variable descriptions to replace nulls
