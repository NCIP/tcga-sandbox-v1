#!perl
#$Id: master_dd_assy.pl 16458 2012-04-20 16:49:00Z jensenma $
use DataDict::DDSearch;
use DataDict::XSD_dbGaP_typemap;
use dbGaP::Config;
use Storable;
use Getopt::Long;
use strict;
...
foreach my $tag (@schemata) {
	...
	# filter types for dbGaP
	if ( defined $typemap->{$$rec{type}} ) {
	  my $typedef = $typemap->{$$rec{type}};
	  if (ref $typedef) {
	    my @td = @$typedef;
	    $$rec{type} = shift @td;
	    $$rec{value} = \@td;
