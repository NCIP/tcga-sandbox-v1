...
## $Source: /cgap/schaefec/cvsroot/TCGA/OverlapExonsAndCNVs.pl,v $
## $Revision: 1.2 $
## $Date: 2009/05/07 19:50:53 $
    ...
    for (my $bin = $start_bin; $bin <= $stop_bin; $bin++) {
      for my $cnv_seg (@{ $bin2cnv{$bin} }) {
        ...
        if ($ov > 0) {
          my $left = $seg->Left;
          my $right = $seg->Right;
          print "$gene\t$chr\t$exon\t$start\t$stop\t$left\t$right\t$ov\n";
        }
      }
    }
