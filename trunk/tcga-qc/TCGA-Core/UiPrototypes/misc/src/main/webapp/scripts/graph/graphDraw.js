	...
	var storeDefaultConfig = {
        ...
        listeners: {
			load: function(store) {
                ...
                if (store.getAt(0).get('name')=='FailErrorFail') {
...
tcga.graph.draw.createGraph = function(paper, store) {
	...
	for (var ndxNodes = 0; ndxNodes < nodeCount;ndxNodes++) {
		 ...
		 */
		sizeH = sizeH * prevConnectingPathSize/prevTotalCount;
	...
	if (store.reader.jsonData.tumorTypes) {
		...
		var results = paper.rect(tumorTypesConfig.pos.x, 1, tumorTypesConfig.width, 100, 5);
		...
		var tumorList = paper.text(labelX, textBase, tcga.graph.widgets.text.splitText(tumorTypesConfig.types, tumorTypesConfig.width - margins*2, {'font-size': tumorTypesConfig.textSize + 'px'}));
	...
	if (store.reader.jsonData.totals) {
		...
		// Get the total of the counts in the totals box - exclude anything with "received" in it
		var totalCount = 0;
