 ...
 *			center - centers each node on the path connecting path to it, false by default
...
tcga.graph.draw.defaultGraphConfig = {
	renderTo: 'raphgraph',
	paperSize: {width: 900, height: 500},
	center: false,
	scale: null,
	threeD: true,
	startPos: {x: 1, y: 350},
		...
		listeners: {
			load: function(store) {
				...
				if (!tcga.graph.draw.paper[graphConfig.renderTo]) {
					tcga.graph.draw.paper[graphConfig.renderTo] = Raphael(graphConfig.renderTo, graphConfig.paperSize.width, graphConfig.paperSize.height);
				}
				else {
...
tcga.graph.draw.createGraph = function(paper, store) {
	...
	for (var ndxNodes = 0; ndxNodes < nodeCount;ndxNodes++) {
		 ...
		 */
		var sizeH = graphConfig.squareSize.width * prevConnectingPathSize/prevTotalCount;
		var sizeV = graphConfig.squareSize.height * prevConnectingPathSize/prevTotalCount;
		var nodeConfig = {
			paper: paper,
			threeD: graphConfig.threeD,
			...
			pathHeight: graphConfig.pathHeight * ((ndxNodes == 0 && graphConfig.scale != null)?nodeTotalCount:prevConnectingPathSize)/(graphConfig.scale != null?graphConfig.scale:prevTotalCount),
		 ...
		 * we just drew.
		 */
		currLoc += (graphConfig.squareSize.width * prevConnectingPathSize/prevTotalCount) + pathLength;
	...
	if (store.reader.jsonData.totals) {
		...
		// Get the total of the counts in the totals box.
		var totalCount = tcga.graph.widgets.totalCount(totalsConfig.counts);
		 ...
		 * 		on the fontsize
		 */
		var textBase = totalsConfig.pos.y + 17;
		...
		var results = paper.rect(totalsConfig.pos.x, 1, totalsConfig.width, (totalsConfig.counts.length * textInterval) + 40);
		results.attr('fill', totalsConfig.fill);
		// Figure out where the dividing line goes - this is between the data and the total
		var dividerLoc = (totalsConfig.counts.length * textInterval) + 11;
