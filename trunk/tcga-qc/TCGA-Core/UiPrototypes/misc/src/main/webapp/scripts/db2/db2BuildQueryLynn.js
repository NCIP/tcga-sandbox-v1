...
tcga.db2.build.createPatientClassSelectors = function(action) {
	// Per Lynn
	Ext.get('patientClassSelection').show();
	Ext.get('molecularSetSelection').hide();
	// End per Lynn
...
tcga.db2.build.createmolecularSetSelectors = function(action) {
	// Per Lynn
	Ext.get('molecularSetSelection').show();
	Ext.get('patientClassSelection').hide();
	// End per Lynn
...
tcga.db2.build.postSaveUpdate = function(queryClass) {
	if (queryClass == 'patient') {
		if (!tcga.db2.selectors.queryType.find('molecular')) {
			Ext.get('buildQuerySelectMolecularData').show();
		}
	
		tcga.db2.selectors.closeAll('patientClass');
	}
	else {
		if (!tcga.db2.selectors.queryType.find('patient')) {
			Ext.get('buildQueryAddPatientClass').show();
		}
	
		tcga.db2.selectors.closeAll('molecularSet');
	}
	tcga.db2.queryWriter.start();
}
...
tcga.db2.build.init = function(){
	...
	// Per Lynn
	Ext.get('buildQueryAddPatientClass').setVisibilityMode(Ext.Element.DISPLAY);
	Ext.get('buildQueryAddPatientClass').hide();
	Ext.get('buildQuerySelectMolecularData').setVisibilityMode(Ext.Element.DISPLAY);
	Ext.get('buildQuerySelectMolecularData').hide();
	Ext.get('patientClassSelection').setVisibilityMode(Ext.Element.DISPLAY);
	Ext.get('molecularSetSelection').setVisibilityMode(Ext.Element.DISPLAY);
	Ext.get('addPatientClass').hide();
	Ext.get('editPatientClass').hide();
	Ext.get('addMolecularSet').hide();
	Ext.get('editMolecularSet').hide();
